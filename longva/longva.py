...
try:
    from LongLLaVA.llava.constants import (
        IMAGE_TOKEN_INDEX,
        DEFAULT_IMAGE_TOKEN,
    )
    from LongLLaVA.llava.conversation import conv_templates, SeparatorStyle
    from LongLLaVA.llava.mm_utils import (
        get_model_name_from_path,
        process_images,
        tokenizer_image_token,
        KeywordsStoppingCriteria,
    )
    from LongLLaVA.llava.model.builder import load_pretrained_model
...
class LongVA:
    ...
    def __init__(
        self,
        pretrained: str = "LongVa/LLaVA-NeXT-Qwen2-7B-extend-avgpool2x2-anyres7x7",
        ...
        if task_type == "image":  # For image task
            ...
            try:
                if self.video_decode_backend == "decord":
                    frames = self.load_video(visuals, self.max_frames_num)
